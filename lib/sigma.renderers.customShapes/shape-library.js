;(function(undefined) {
  ...
  var drawRoundedRectangle = function(node,x,y,size,color,context) {
    ...
    var x1 = x-size, y1 = y-size, font = 12;
    context.fillStyle = color;
    roundedRectangle(x1,y1,width,height,cornerRadius,context);
    context.stroke();
    //context.beginPath();
    //context.moveTo(x1, y1);
    //context.lineTo(x1+width, y1);
    //context.quadraticCurveTo(x1+(side-cornerRadius), y1, x1+(side-cornerRadius), y1+cornerRadius);
    //context.lineTo(x1+(side-cornerRadius), y1+height);
    //context.quadraticCurveTo(x1+(side-cornerRadius), y1+(side-cornerRadius), x1+width, y1+(side-cornerRadius));
    //context.lineTo(x1, y1+(side-cornerRadius));
    //context.quadraticCurveTo(x1-cornerRadius, y1+(side-cornerRadius), x1-cornerRadius, y1+width);
    //context.lineTo(x1-cornerRadius, y1+cornerRadius);
    //context.quadraticCurveTo(x1-cornerRadius, y1, x1, y1);
    //context.stroke();
    var grd = context.createLinearGradient(x1, y1+side,x1,y1);
    ...
    wrapText(context,node.label, x1+2, y1+(height/2), side-7, font+2,'.');
    //draw belief
  ...
  function roundedRectangle (x, y, w, h, r,context) {
    if (w < 2 * r) r = w / 2;
    if (h < 2 * r) r = h / 2;
    context.beginPath();
