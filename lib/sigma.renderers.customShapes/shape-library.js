;(function(undefined) {
  ...
  var drawRoundedRectangle = function(node,x,y,size,color,context) {
    ...
    wrapText(context,node.label, x1+2, y1+(height/2)+5, side-7, font+2,'.');

    //draw belief
    if(node.type!=="Primary Skill" && node.type!=="Assessment"){
      context.fillStyle = "#FFF";
      roundedRectangle(x1+25,y1-10,60,15,3,context);
      context.stroke();
      context.fillStyle = "#C0504D";
      context.fill();
      context.fillStyle = "#FFF";
      ...
      context.fillText(node.belief+" [~"+node.deviation+"]", x1+28, y1+1);
    }
